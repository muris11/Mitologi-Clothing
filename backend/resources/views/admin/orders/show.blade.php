    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-premium border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="border-b border-gray-100 dark:border-gray-700 bg-amber-50/50 dark:bg-amber-900/10 p-6 relative">
                    <div class="absolute top-0 left-0 w-1 h-full bg-amber-500"></div>
                    <h3 class="flex items-center gap-2 text-base font-bold text-gray-900 dark:text-white tracking-tight">
                ...
                <div class="p-6 space-y-6">
                    <div class="bg-white dark:bg-gray-900 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                        <p class="text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Alasan Pelanggan:</p>
                        <p class="text-base text-gray-600 dark:text-gray-400 italic">"{{ $order->refund_reason }}"</p>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <form action="{{ route('admin.orders.reject-refund', $order) }}" method="POST" class="space-y-4 mt-2">
                            ...
                            <button type="submit" onclick="return confirm('Yakin ingin menolak refund? Pesanan akan berlanjut diproses.')" class="w-full px-4 py-2.5 bg-white dark:bg-gray-800 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 shadow-sm transition-colors font-semibold flex justify-center items-center gap-2">
            ...
            <div class="admin-panel overflow-hidden">
                <div class="p-6 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/30">
                    <h3 class="text-lg font-bold text-mitologi-navy dark:text-white uppercase tracking-tight">Item Pesanan</h3>
                </div>
                
                <!-- Desktop Table -->
                <div class="hidden md:block overflow-x-auto">
                    <table class="w-full text-left text-sm text-gray-600 dark:text-gray-400">
                        <thead class="bg-gray-50 dark:bg-gray-700/50 text-xs uppercase font-bold text-gray-500 dark:text-gray-300 tracking-wider">
                            <tr>
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="h-12 w-12 rounded-lg bg-gray-100 dark:bg-gray-700 overflow-hidden border border-gray-200 dark:border-gray-600 mr-4 flex-shrink-0">
                                        ...
                                        <div>
                                            <div class="font-bold text-mitologi-navy dark:text-white">{{ $item->product->title ?? 'Product Deleted' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right font-mono">Rp {{ number_format($item->price, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-center">{{ $item->quantity }}</td>
                                <td class="px-6 py-4 text-right font-mono font-bold text-mitologi-navy dark:text-white">Rp {{ number_format($item->total, 0, ',', '.') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="md:hidden divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <div class="p-4 bg-white dark:bg-gray-800">
                        <div class="flex gap-4">
                            <div class="h-16 w-16 rounded-lg bg-gray-50 dark:bg-gray-700 overflow-hidden border border-gray-100 dark:border-gray-600 flex-shrink-0">
                                @if($item->product && $item->product->featured_image)
                                    <img src="{{ asset('storage/'.$item->product->featured_image) }}" alt="{{ $item->product->title }}" class="h-full w-full object-cover">
                                @else
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="font-bold text-mitologi-navy dark:text-white text-sm mb-1 leading-tight">{{ $item->product->title ?? 'Produk Dihapus' }}</div>
            ...
            <div class="admin-panel p-6">
                <h3 class="text-lg font-bold text-mitologi-navy dark:text-white mb-6">Ringkasan Pembayaran</h3>
                <div class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div class="flex justify-between items-center border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
                        <span class="text-base font-bold text-mitologi-navy dark:text-white">Total Bayar</span>
        ...
        <div class="space-y-6">
            ...
            <div class="admin-panel p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                             <div class="font-bold text-mitologi-navy dark:text-white">{{ $order->user->name ?? 'Guest' }}</div>
            ...
            <div class="admin-panel p-6">
                ...
                <div class="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/30 p-4 rounded-lg">
                    @if($order->shippingAddress)
                        <p class="font-bold text-mitologi-navy dark:text-white mb-1">{{ $order->shippingAddress->first_name }} {{ $order->shippingAddress->last_name }}</p>
            ...
            <div class="admin-panel p-6">
                ...
                @if(in_array($order->status, ['refunded', 'cancelled']))
                    <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-5 text-center border border-gray-200 dark:border-gray-600">
                    ...
                    <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                <select name="status" id="status" class="block w-full pl-3 pr-10 py-2.5 text-base border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-mitologi-navy focus:border-mitologi-navy sm:text-sm rounded-lg dark:bg-gray-900 dark:text-white">
