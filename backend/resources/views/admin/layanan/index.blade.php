<x-admin-layout>
            ...
            <x-admin-card>
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <h3 class="text-lg font-bold text-gray-900 ">Daftar Layanan</h3>
                    </div>
                ...
                <div class="bg-gray-50/50  border border-gray-100  rounded-xl p-4 mb-6 flex gap-3 text-sm text-gray-500 ">
            ...
            <x-admin-card class="!p-0 border-0">
                <div class="p-6 bg-gray-50/50  flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-gray-900 ">Pricelist Kategori Produk</h3>
                            <p class="text-xs text-gray-500  mt-0.5">Kelola daftar harga per kategori produk</p>
                        </div>
                    </div>
                    <a href="{{ route('admin.beranda.product-pricings.index') }}" class="flex-shrink-0 px-4 py-2 border border-gray-200  bg-white  text-mitologi-navy  text-sm font-bold rounded-lg hover:bg-gray-50  transition-colors shadow-sm flex items-center gap-2">
                        Kelola
            ...
            <x-admin-card class="!p-0 border-0">
                <div class="p-6 bg-gray-50/50  flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-gray-900 ">Metode Sablon (Printing Methods)</h3>
                            <p class="text-xs text-gray-500  mt-0.5">Kelola daftar metode sablon yang tersedia</p>
                        </div>
                    </div>
                    <a href="{{ route('admin.layanan.printing-methods.index') }}" class="flex-shrink-0 px-4 py-2 border border-gray-200  bg-white  text-mitologi-navy  text-sm font-bold rounded-lg hover:bg-gray-50  transition-colors shadow-sm flex items-center gap-2">
                        Kelola
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    </a>
                </div>
    ...
    const inputClass = "w-full px-4 py-2 border border-gray-300  bg-white  text-gray-900  focus:border-mitologi-navy focus:ring-1 focus:ring-mitologi-navy rounded-lg shadow-sm placeholder-gray-400 text-sm transition-colors";
    const textareaClass = "w-full px-4 py-3 border border-gray-300  bg-white  text-gray-900  focus:border-mitologi-navy focus:ring-1 focus:ring-mitologi-navy rounded-lg shadow-sm placeholder-gray-400 text-sm transition-colors";
    ...
    function renderServices() {
        ...
        if(!servicesData.length){
            c.innerHTML='<div class="py-12 text-center text-gray-400 bg-gray-50/50  rounded-xl border-2 border-dashed border-gray-200 "><svg class="w-12 h-12 mx-auto mb-3 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg><p>Belum ada layanan.</p><button type="button" onclick="addService()" class="mt-2 text-sm text-mitologi-navy hover:text-mitologi-gold-dark transition-colors font-medium">Tambah layanan baru</button></div>';
            return;
        }
        c.innerHTML = servicesData.map((s,i) => `
        <div class="relative bg-white  p-6 rounded-xl border border-gray-200  shadow-sm hover:shadow-lg transition-all duration-300 hover:border-mitologi-navy/30 ">
            <div class="absolute top-4 right-4 z-10 opacity-100">
                 <button type="button" onclick="removeService(${i})" class="bg-white  text-red-500 hover:text-red-700 p-2 rounded-lg shadow-sm hover:bg-red-50  transition-colors border border-gray-100 ">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-gray-500  uppercase tracking-wider mb-2">Judul Layanan</label>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            ${s.image ? `<img src="${s.image.startsWith('http')?s.image:'/storage/'+s.image}" class="h-16 w-16 object-cover rounded-lg border border-gray-200  shadow-sm">` : '<div class="h-16 w-16 bg-gray-100  rounded-lg flex items-center justify-center text-gray-400"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg></div>'}
                            <label class="cursor-pointer bg-white  text-mitologi-navy  border border-gray-300  rounded-lg px-4 py-2 text-sm font-medium hover:bg-gray-50  transition-colors shadow-sm">
                ...
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100 ">
                    <div>
                        <label class="block text-xs font-bold text-gray-500  uppercase tracking-wider mb-2">Material (pisahkan dengan koma)</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500  uppercase tracking-wider mb-2">Keunggulan (pisahkan dengan koma)</label>
