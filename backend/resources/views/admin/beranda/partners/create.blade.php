<x-admin-layout>
    <x-admin-header
        ...
        :breadcrumbs="[['title' => 'Beranda', 'url' => route('admin.beranda.index')], ['title' => 'Partner', 'url' => route('admin.beranda.partners.index')], ['title' => 'Tambah Baru']]"
    />

    <form action="{{ route('admin.beranda.partners.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                ...
                <x-admin-card>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="website_url" class="block mt-1 w-full" type="url" name="website_url" :value="old('website_url')" placeholder="https://..." />
                            <x-input-error :messages="$errors->get('website_url')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="description" :value="__('Deskripsi Pendek (Opsional)')" />
                            <textarea id="description" name="description" rows="3" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-mitologi-gold dark:focus:border-mitologi-gold focus:ring-mitologi-gold dark:focus:ring-mitologi-gold rounded-md shadow-sm">{{ old('description') }}</textarea>
                ...
                <x-admin-card>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <x-file-upload name="logo" label="Upload Logo" />
