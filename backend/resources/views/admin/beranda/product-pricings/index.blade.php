    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-premium overflow-hidden border border-gray-100 dark:border-gray-700">
            ...
            <table class="w-full text-left text-sm text-gray-600 dark:text-gray-400">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                    ...
                    <tr class="hover:bg-mitologi-cream/30 dark:hover:bg-gray-700/30 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="font-bold text-mitologi-navy dark:text-white text-base group-hover:text-mitologi-gold transition-colors">{{ $pricing->category_name }}</div>
                            <div class="text-xs text-gray-500 truncate max-w-[250px] mt-1">{{ Str::limit($pricing->notes, 80) }}</div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $pricing->min_order ?? '-' }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-1 max-h-24 overflow-y-auto pr-2">
                                @if(is_array($pricing->items))
                                    @foreach($pricing->items as $item)
                                        <div class="text-xs flex justify-between gap-4 border-b border-gray-100 dark:border-gray-700/50 pb-1 last:border-0 last:pb-0">
                        ...
                        <td class="px-6 py-4">
                            <span class="font-mono text-gray-500 dark:text-gray-400">#{{ $pricing->sort_order }}</span>
                        </td>
                        <td class="px-6 py-4">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500 bg-gray-50/30 dark:bg-gray-800/50">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 dark:text-white">Belum ada pricelist</h3>
        ...
        <div class="md:hidden divide-y divide-gray-100 dark:divide-gray-700/50">
            ...
            <div class="p-4">
                ...
                <div class="flex items-start justify-between gap-3 mb-3">
                    <div class="flex-1 min-w-0">
                        <div class="font-bold text-mitologi-navy dark:text-white text-sm">{{ $pricing->category_name }}</div>
                        @if($pricing->notes)
                ...
                <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2.5 mb-3 space-y-1.5">
                    @foreach(array_slice($pricing->items, 0, 3) as $item)
                    <div class="flex justify-between text-xs">
                        <span class="font-medium text-gray-700 dark:text-gray-300 truncate max-w-[55%]">{{ $item['name'] ?? '' }}</span>
                        <span class="text-gray-500 font-mono">{{ $item['price_range'] ?? '' }}</span>
                    </div>
                    @endforeach
            ...
            <div class="p-8 text-center">
                <div class="p-4 rounded-full bg-gray-100 dark:bg-gray-700 w-fit mx-auto mb-3">
                ...
                <h3 class="text-base font-medium text-gray-900 dark:text-white">Belum ada pricelist</h3>
                <p class="text-sm text-gray-500 mt-1 mb-4">Tambahkan kategori pricelist pertama.</p>
                <a href="{{ route('admin.beranda.product-pricings.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-mitologi-navy hover:bg-mitologi-navy-light rounded-lg shadow-sm">
                    Tambah Pricelist
                </a>
