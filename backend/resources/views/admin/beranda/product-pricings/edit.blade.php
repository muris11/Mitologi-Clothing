<x-admin-layout>
    <x-admin-header
        title="Edit Pricelist Kategori"
        :breadcrumbs="[['title' => 'Beranda', 'url' => route('admin.beranda.index')], ['title' => 'Pricelist', 'url' => route('admin.beranda.product-pricings.index')], ['title' => 'Edit: ' . $pricing->category_name]]"
    />

    <form action="{{ route('admin.beranda.product-pricings.update', $pricing->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-8">
                ...
                <x-admin-card>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <x-text-input id="category_name" class="block mt-1 w-full" type="text" name="category_name" :value="old('category_name', $pricing->category_name)" required />
                            <x-input-error :messages="$errors->get('category_name')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="min_order" :value="__('Minimum Order')" />
                            <x-text-input id="min_order" class="block mt-1 w-full" type="text" name="min_order" :value="old('min_order', $pricing->min_order)" />
                ...
                <x-admin-card>
                    ...
                    @php
                        $itemsData = is_array($pricing->items) ? $pricing->items : [];
                        if (empty($itemsData)) {
                            $itemsData = [['name' => '', 'price_range' => '']];
                        }
                        $oldItems = old('items', $itemsData);
                    @endphp
                ...
                <x-admin-card>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <x-text-input id="sort_order" class="block mt-1 w-full" type="number" name="sort_order" :value="old('sort_order', $pricing->sort_order)" />
