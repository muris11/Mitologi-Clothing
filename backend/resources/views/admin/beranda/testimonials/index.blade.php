                    ...
                    <tr class="hover:bg-mitologi-cream/30 dark:hover:bg-gray-700/30 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="h-10 w-10 rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden shadow-sm mr-3 flex-shrink-0 border border-gray-200 dark:border-gray-600">
                                ...
                                <div>
                                    <div class="font-bold text-mitologi-navy dark:text-white">{{ $testimonial->name }}</div>
                                    <div class="text-xs text-gray-500">{{ $testimonial->role }}</div>
                                </div>
                            </div>
                        </td>
                         <td class="px-6 py-4 max-w-sm">
                            <p class="text-gray-600 dark:text-gray-300 truncate italic">"{{ $testimonial->content }}"</p>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex text-mitologi-gold">
                                @for($i = 0; $i < 5; $i++)
                                    <svg class="w-4 h-4 {{ $i < $testimonial->rating ? 'fill-current' : 'text-gray-300 dark:text-gray-600' }}" viewBox="0 0 20 20"><path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/></svg>
                                @endfor
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-3">
                                <a href="{{ route('admin.beranda.testimonials.edit', $testimonial) }}" class="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white p-1.5 text-mitologi-navy shadow-sm transition-colors hover:bg-gray-100 hover:text-mitologi-gold dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white">
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500 bg-gray-50/30 dark:bg-gray-800/50">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 dark:text-white">Belum ada testimonial</h3>
            ...
            @forelse($testimonials as $testimonial)
                <div class="p-4 flex flex-col gap-3 bg-white dark:bg-gray-800">
                    <div class="flex items-start gap-4">
                        <div class="h-12 w-12 rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden shadow-sm flex-shrink-0 border border-gray-200 dark:border-gray-600">
                            @if($testimonial->avatar_url)
                                <img src="{{ asset('storage/' . $testimonial->avatar_url) }}" alt="{{ $testimonial->name }}" class="h-full w-full object-cover">
                            @else
                                <div class="h-full w-full flex items-center justify-center text-white font-bold bg-gradient-to-br from-mitologi-navy to-mitologi-navy-light text-sm">
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 class="font-bold text-mitologi-navy dark:text-white text-base">{{ $testimonial->name }}</h3>
