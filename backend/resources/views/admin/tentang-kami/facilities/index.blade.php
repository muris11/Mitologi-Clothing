                    ...
                    <tr class="hover:bg-mitologi-cream/30  transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="h-16 w-24 rounded-lg bg-gray-100  overflow-hidden shadow-sm mr-4 flex-shrink-0 border border-gray-200  relative group-hover:shadow-md transition-shadow">
                                    @if($facility->image)
                                ...
                                <div>
                                    <div class="font-bold text-mitologi-navy  text-base group-hover:text-mitologi-gold transition-colors">{{ $facility->name }}</div>
                                    <div class="text-xs text-gray-500 truncate max-w-[250px]">{{ Str::limit($facility->description, 80) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="font-mono text-gray-500 ">#{{ $facility->sort_order }}</span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-3">
                                <a href="{{ route('admin.tentang-kami.facilities.edit', $facility->id) }}" class="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white p-1.5 text-mitologi-navy shadow-sm transition-colors hover:bg-gray-100 hover:text-mitologi-gold     ">
            ...
            @forelse($facilities as $facility)
                <div class="p-4 flex flex-col gap-3 bg-white ">
                    <div class="flex items-center gap-3">
                        <div class="h-20 w-24 rounded-lg bg-gray-100  overflow-hidden shadow-sm flex-shrink-0 border border-gray-200 ">
                            @if($facility->image)
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between items-start">
                                <h3 class="font-bold text-mitologi-navy  text-base">{{ $facility->name }}</h3>
