...
<x-admin-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        <!-- Chart Section -->
        <div class="lg:col-span-2 bg-white  rounded-2xl shadow-premium p-8">
            <div class="flex items-center justify-between mb-8">
                <div>
                     <h5 class="text-xl font-bold text-mitologi-navy ">Grafik Penjualan</h5>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-medium text-gray-600 ">30 Hari Terakhir</span>
                </div>
            </div>
        ...
        <!-- Recent Orders -->
        <div class="bg-white  rounded-2xl shadow-premium p-0 flex flex-col overflow-hidden h-full">
             <div class="p-6 border-b border-gray-100  bg-gray-50/50  flex justify-between items-center">
                <h5 class="text-lg font-bold text-mitologi-navy ">Pesanan Terbaru</h5>
            ...
            <div class="overflow-y-auto scrollbar-hide flex-1 p-4">
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-4 rounded-xl bg-white  border border-gray-100  hover:border-mitologi-gold/50 hover:shadow-md transition-all group">
                        <div class="flex items-center space-x-4">
                             @if($order->user && $order->user->avatar)
                                <img src="{{ asset('storage/' . $order->user->avatar) }}" alt="{{ $order->user->name }}" class="w-12 h-12 rounded-full object-cover border-2 border-gray-100  group-hover:border-mitologi-gold transition-colors">
                             @else
                                <div class="w-12 h-12 rounded-full bg-mitologi-navy/5 text-mitologi-navy   flex items-center justify-center font-bold text-lg group-hover:bg-mitologi-navy group-hover:text-white transition-colors">
                                {{ substr($order->user->name ?? 'G', 0, 1) }}
                            </div>
                             @endif
                            <div>
                                <h6 class="text-sm font-bold text-gray-800  group-hover:text-mitologi-navy  transition-colors">{{ $order->user->name ?? 'Tamu' }}</h6>
                                <p class="text-xs text-gray-500">Order #{{ $order->order_number ?? $order->id }}</p>
                            </div>
                        </div>
                         <div class="text-right">
                             <p class="text-sm font-bold text-mitologi-navy ">Rp {{ number_format($order->total, 0, ',', '.') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white  rounded-2xl shadow-premium overflow-hidden">
             <div class="p-6 border-b border-gray-100  bg-gray-50/50  flex justify-between items-center">
                <h5 class="text-lg font-bold text-mitologi-navy ">Produk Terlaris</h5>
                            ...
                            <tr class="hover:bg-gray-50  transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h6 class="text-sm font-bold text-mitologi-navy  group-hover:text-mitologi-gold transition-colors">{{ $item->product ? $item->product->title : 'Produk Dihapus' }}</h6>
                ...
                <div class="md:hidden divide-y divide-gray-100 ">
                    @forelse($topProducts as $item)
                        <div class="p-4 flex items-center gap-4 bg-white ">
                            <div class="w-16 h-16 rounded-xl bg-gray-50  overflow-hidden flex-shrink-0 border border-gray-100 ">
                                @if($item->product && $item->product->featured_image)
                                    <img src="{{ asset('storage/'.$item->product->featured_image) }}" class="w-full h-full object-cover">
                                @else
                            ...
                            <div class="flex-1 min-w-0">
                                <h6 class="text-sm font-bold text-mitologi-navy  truncate">{{ $item->product ? $item->product->title : 'Produk Dihapus' }}</h6>
                                <div class="flex items-center justify-between mt-1">
                                    <span class="text-xs font-bold text-mitologi-gold">{{ $item->total_sold }} Terjual</span>
                                    <span class="text-sm font-black text-gray-900 ">Rp {{ number_format($item->total_revenue, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
                    @empty
        ...
        <div class="space-y-8 h-full flex flex-col">
            ...
            <div class="bg-white  rounded-2xl shadow-premium overflow-hidden border-t-4 border-red-500">
                ...
                <div class="divide-y divide-gray-100 ">
                    ...
                    <div class="p-3 flex items-center justify-between hover:bg-red-50/50 transition-colors">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="text-xs font-bold text-mitologi-navy  truncate max-w-[120px]">{{ $variant->product ? $variant->product->title : 'Produk Dihapus' }}</p>
            ...
            <!-- Latest Testimonials -->
            <div class="bg-white  rounded-2xl shadow-premium overflow-hidden flex-1">
                 <div class="p-5 border-b border-gray-100  flex justify-between items-center bg-gray-50/50 ">
                    <h5 class="text-base font-bold text-mitologi-navy ">Ulasan Terbaru</h5>
                ...
                <div class="p-4 space-y-4">
                    ...
                    <div class="p-4 rounded-xl border border-gray-100  bg-white  relative">
                        ...
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            @endif
                            <h6 class="text-sm font-bold text-mitologi-navy ">{{ $testimonial->name }}</h6>
                        </div>
                        <p class="text-xs text-gray-600  italic line-clamp-2">"{{ $testimonial->content }}"</p>
                    </div>
                    @empty
