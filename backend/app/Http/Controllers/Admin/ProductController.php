...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        FrontendCacheService::revalidate(['products', 'best-sellers', 'new-arrivals', 'categories', 'landing-page']);
        ...
        if ($request->hasFile('image')) {
            ...
            if ($product->featured_image && Storage::disk('public')->exists($product->featured_image)) {
                try {
                    ...
                    Log::info('Featured image deleted', ['product_id' => $product->id, 'path' => $product->featured_image]);
                } catch (\Exception $e) {
                    Log::error('Failed to delete featured image', ['product_id' => $product->id, 'path' => $product->featured_image, 'error' => $e->getMessage()]);
                }
            } else {
                Log::info('Featured image not found for deletion', ['product_id' => $product->id, 'path' => $product->featured_image]);
            }
            $data['featured_image'] = $request->file('image')->store('products', 'public');
        }
        ...
        if ($request->has('deleted_images') && is_array($request->deleted_images)) {
            ...
            foreach ($imagesToDelete as $img) {
                if ($img instanceof \App\Models\ProductImage) {
                    // Delete from storage with existence check
                    if (Storage::disk('public')->exists($img->url)) {
                        try {
                            Storage::disk('public')->delete($img->url);
                            Log::info('Gallery image deleted', ['product_id' => $product->id, 'image_id' => $img->id, 'path' => $img->url]);
                        } catch (\Exception $e) {
                            Log::error('Failed to delete gallery image', ['product_id' => $product->id, 'image_id' => $img->id, 'path' => $img->url, 'error' => $e->getMessage()]);
                        }
                    } else {
                        Log::info('Gallery image file not found for deletion', ['product_id' => $product->id, 'image_id' => $img->id, 'path' => $img->url]);
                    }
                    $img->delete();
                }
            }
    ...
    public function destroy(Product $product)
    {
        ...
        // Delete featured image with existence check and logging
        if ($product->featured_image) {
            if (Storage::disk('public')->exists($product->featured_image)) {
                try {
                    Storage::disk('public')->delete($product->featured_image);
                    Log::info('Product featured image deleted', ['product_id' => $productId, 'title' => $productTitle, 'path' => $product->featured_image]);
                } catch (\Exception $e) {
                    Log::error('Failed to delete product featured image', ['product_id' => $productId, 'title' => $productTitle, 'path' => $product->featured_image, 'error' => $e->getMessage()]);
                }
            } else {
                Log::info('Product featured image not found', ['product_id' => $productId, 'title' => $productTitle, 'path' => $product->featured_image]);
            }
        }

        // Delete gallery images with existence check and logging
        $galleryImages = $product->images;
        foreach ($galleryImages as $img) {
            if (Storage::disk('public')->exists($img->url)) {
                try {
                    Storage::disk('public')->delete($img->url);
                    Log::info('Product gallery image deleted', ['product_id' => $productId, 'title' => $productTitle, 'image_id' => $img->id, 'path' => $img->url]);
                } catch (\Exception $e) {
                    Log::error('Failed to delete product gallery image', ['product_id' => $productId, 'title' => $productTitle, 'image_id' => $img->id, 'path' => $img->url, 'error' => $e->getMessage()]);
                }
            } else {
                Log::info('Product gallery image not found', ['product_id' => $productId, 'title' => $productTitle, 'image_id' => $img->id, 'path' => $img->url]);
            }
        }

        $product->variants()->delete();
        $product->delete();
        Log::info('Product deleted from database', ['product_id' => $productId, 'title' => $productTitle]);
