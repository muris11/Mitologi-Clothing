...
class ChatbotController extends Controller
{
    public function chat(Request $request)
    {
        ...
        if (empty($apiKeys)) {
            return response()->json(['reply' => 'Chatbot sedang dalam mode demo. Mohon konfigurasi GROQ_API_KEYS di backend .env untuk mengaktifkan AI.']);
        }
        
        $apiKey = trim($apiKeys[array_rand($apiKeys)]);

        try {
            // 1. Fetch Product Data for Context (Limit to 30 to save tokens)
            $products = \App\Models\Product::with(['variants', 'categories'])
                ...
                ->map(function($p) {
                    ...
                    $stock = $p->variants->sum('stock'); // Total stock across variants
            ...
            if ($response->successful()) {
                ...
                return response()->json(['error' => 'Gagal menghubungi layanan chatbot.'], 502);
            }
