...
function extractBackendRoutes() {
    if (!fs_1.default.existsSync(API_ROUTES_FILE))
        return [];
...
function extractFrontendCoveredEndpoints() {
    if (!fs_1.default.existsSync(ENDPOINTS_FILE))
        return [];
        ...
        if (stat.isDirectory()) {
            ...
            if (content.includes('fetch(') || content.includes('fetch (')) {
                ...
                if (content.includes('process.env.NEXT_PUBLIC_API_URL')) {
                    issues.push("Direct backend fetch found: ".concat(path_1.default.relative(FRONTEND_DIR, fullPath)));
                }
            }
        }
    }
...
function main() {
    ...
    console.log("Found ".concat(coveredRoutes.length, " covered endpoint wrappers."));
    ...
    if (gaps.length === 0) {
        ...
        gaps.forEach(function (g) { return console.log("\u26A0\uFE0F  ".concat(g)); });
        console.log("Note: Wildcard matching might not be 100% exact in this simple script.");
    }
    console.log("\n--- Direct Fetch Violations ---");
    var searchDirs = [path_1.default.join(FRONTEND_DIR, 'app'), path_1.default.join(FRONTEND_DIR, 'components')];
    var violations = [];
    ...
    if (violations.length === 0) {
        ...
        violations.forEach(function (v) { return console.log("\uD83D\uDEAB ".concat(v)); });
    }
    console.log("\n================================");
}
main();
