        ...
        if (stat.isDirectory()) {
            ...
            if (content.includes('fetch(') || content.includes('fetch (')) {
                ...
                if (content.includes('process.env.NEXT_PUBLIC_API_URL')) {
                    issues.push(`Direct backend fetch found: ${path.relative(FRONTEND_DIR, fullPath)}`);
                }
            }
        }
    }
...
function main() {
    ...
    const searchDirs = [path.join(FRONTEND_DIR, 'app'), path.join(FRONTEND_DIR, 'components')];
