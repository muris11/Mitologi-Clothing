...
export function WishlistTab() {
  ...
  useEffect(() => {
    ...
    getWishlist()
      .then(setWishlist)
      .catch(() => addToast({ variant: "error", title: "Gagal memuat wishlist" }))
  ...
  return (
    <div className="space-y-6">
        <div className="border-b border-slate-100 pb-6">
            <h2 className="text-2xl font-sans font-bold text-mitologi-navy mb-2">Wishlist Saya</h2>
            <p className="text-sm font-sans text-slate-500">Produk yang Anda simpan untuk dibeli nanti.</p>
        </div>
        ...
        {isLoadingWishlist ? (
           <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
             ...
             <div className="text-center py-20 bg-slate-50 rounded-3xl border border-dashed border-slate-200 shadow-sm">
                ...
                <h3 className="text-xl font-sans font-bold text-mitologi-navy mb-2">Wishlist kosong</h3>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
                {wishlist.map((product) => {
                    const imageUrl = storageUrl(product.featuredImage?.url, "/images/placeholder.jpg");

                    return (
                    <div key={product.id} className="group relative bg-white rounded-3xl border border-slate-100 overflow-hidden hover:shadow-lg hover:border-slate-200 transition-all duration-300 hover:-translate-y-1">
                         ...
                         <div className="p-6 flex flex-col justify-between h-36">
                             <div>
                                <h3 className="font-sans font-bold text-lg text-mitologi-navy line-clamp-1 mb-1 group-hover:text-mitologi-gold transition-colors">
                                    <Link href={`/shop/product/${product.handle}`} className="absolute inset-0 z-0" aria-label={`View ${product.title}`}></Link>
                                    {product.title}
                                </h3>
                                <p className="font-sans font-medium text-slate-500 text-sm">
                                    {new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(Number(product.priceRange.minVariantPrice.amount))}
                                </p>
                             </div>
                             
                             <div className="relative z-10 mt-4">
                                <Button asChild variant="secondary" className="w-full shadow-sm" size="sm">
                                    <Link href={`/shop/product/${product.handle}`}>Lihat Produk</Link>
                                </Button>
                             </div>
                         </div>
                    </div>
