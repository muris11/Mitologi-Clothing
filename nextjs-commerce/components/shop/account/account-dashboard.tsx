...
import { ArrowRightOnRectangleIcon, Bars3Icon, ChevronRightIcon, Cog6ToothIcon, HeartIcon, ShoppingBagIcon, UserCircleIcon, XMarkIcon } from "@heroicons/react/24/outline";
...
export function AccountDashboard({ user, orders }: { user: User; orders: Order[] }) {
  ...
  const totalSpend = orders
    .filter(o => ['processing', 'completed', 'shipped', 'paid'].includes(o.status))
    .reduce((sum, order) => sum + Number(order.total || 0), 0);
  const userSince = new Date(user.created_at || Date.now()).getFullYear();
  ...
  return (
    <div className="lg:grid lg:grid-cols-12 min-h-[700px] bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-200">
      ...
      <div className="lg:hidden p-4 border-b border-slate-200 flex justify-between items-center bg-white">
        <span className="font-sans font-bold text-mitologi-navy text-sm">Menu Akun</span>
        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="p-2 text-slate-500 hover:bg-slate-50 hover:text-mitologi-navy rounded-xl border border-slate-200 transition-colors shadow-sm">
        ...
        <div className="relative z-10 space-y-2">
            <div className="mb-6 px-2 flex items-center gap-4">
                 ...
                 <div className="overflow-hidden">
                     <p className="font-sans font-bold text-sm text-mitologi-navy truncate">{user.name}</p>
                     <p className="text-sm text-slate-500 font-sans truncate">{user.email}</p>
                 </div>
            </div>

            <div className="mb-4 px-3">
                <h3 className="text-xs font-sans font-bold text-slate-400 uppercase tracking-wider mb-2">Menu Utama</h3>
            </div>
              ...
              >
                <div className="flex items-center gap-3">
                    <Icon className={clsx(
                        ...
                        isActive ? "text-mitologi-gold" : "text-slate-400 group-hover:text-mitologi-navy"
                    )} />
                    <span>{tab.label}</span>
                </div>
                {isActive && (
          ...
          <div className="pt-6 mt-6 border-t border-slate-200 px-2">
              ...
              >
                ...
                <span className="truncate">{isLoggingOut ? "Keluar…" : "Keluar Aplikasi"}</span>
              </button>
          </div>
        </div>
      </aside>
      ...
      <div className="lg:col-span-9 bg-white p-6 lg:p-10 relative">
          <div className="h-full">
              {activeTab === "profile" && <ProfileTab user={user} totalOrders={totalOrders} totalSpend={totalSpend} userSince={userSince} />}
