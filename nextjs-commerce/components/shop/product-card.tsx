...
export const ProductCard = memo(function ProductCard({ product, index = 0, isRecommended, isBestSeller }: ProductCardProps) {
  ...
  const variantTitle = hasMultipleVariants ? `${variants.length} Varian` : (firstVariant?.title !== 'Default Title' ? firstVariant?.title : '');
  ...
  const hasSoldMetric = typeof product.totalSold === "number" && product.totalSold > 0;
  const hasRating = typeof product.averageRating === "number" && product.averageRating > 0;
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-[22px] shadow-soft hover:shadow-hover border border-app overflow-hidden transition-shadow duration-200 group">
      ...
      <div className="relative aspect-[4/4.8] overflow-hidden bg-app-cream">
        <Link href={`/shop/product/${product.handle}`} className="block h-full w-full">
            {imageUrl ? (
              <Image
                src={imageUrl}
      ...
      <div className="flex flex-col flex-grow p-4 bg-white relative">
        ...
        <div className="flex min-h-[18px] items-center gap-1.5 text-[11px] text-slate-500 font-sans mt-1">
            {hasRating && <div className="flex items-center">
                <svg className="w-3 h-3 text-mitologi-gold mr-0.5" fill="currentColor" viewBox="0 0 20 20">
                   <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <span>{product.averageRating?.toFixed(1)}</span>
            </div>}
            {hasRating && hasSoldMetric && <span className="w-0.5 h-0.5 rounded-full bg-slate-300"></span>}
        ...
        <div className="flex items-center justify-between text-[11px] text-slate-400 font-sans mt-2">
           <div className="truncate">{variantTitle ? <span>{variantTitle}</span> : <span>Siap dikirim</span>}</div>
