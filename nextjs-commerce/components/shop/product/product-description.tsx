...
export function ProductDescription({ product }: { product: Product }) {
  ...
  const [prevVariantId, setPrevVariantId] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const hasSelectedVariantInParams = product.variants.some((variant) => {
      const selectedOptions = Array.isArray(variant.selectedOptions) ? variant.selectedOptions : [];
      return selectedOptions.length > 0 && selectedOptions.every((opt) => searchParams.get(opt.name.toLowerCase()) === opt.value);
    });
    ...
    const selectedOptions = Array.isArray(defaultVariant?.selectedOptions) ? defaultVariant.selectedOptions : [];
  ...
  const handleQuantityChange = (type: "plus" | "minus") => {
    if (type === "plus") {
      setQuantity((prev) => {
        ...
        if (matchedVariant?.stock !== undefined && prev >= matchedVariant.stock) {
          return prev;
        }
        return prev + 1;
      });
    ...
    <div className="flex flex-col h-full font-sans">
        ...
        <div className="flex items-center gap-4 text-sm font-sans font-medium text-slate-500 mb-6 flex-wrap">
          {product.averageRating && product.averageRating > 0 ? (
            <div className="flex items-center gap-1.5 bg-amber-50 px-2 py-1 rounded-md text-amber-700">
              ...
              <div className="flex text-amber-500">
                {[...Array(5)].map((_, i) => (
                  <svg key={i} className={`w-4 h-4 ${i < Math.round(product.averageRating!) ? 'fill-current' : 'fill-slate-200 text-slate-200'}`} viewBox="0 0 20 20">
            ...
            <span className="text-slate-400 italic text-xs">Belum ada ulasan</span>
          )}
          {(product.totalReviews ?? 0) > 0 && (
            <>
              <div className="w-1 h-1 rounded-full bg-slate-300"></div>
              <div className="flex items-center gap-1">
                <span className="font-bold text-mitologi-navy">{product.totalReviews}</span>
                <span>Penilaian</span>
              </div>
            </>
          )}
          {(product.totalSold ?? 0) > 0 && (
            <>
              <div className="w-1 h-1 rounded-full bg-slate-300"></div>
              <div className="flex items-center gap-1">
                <span className="font-bold text-mitologi-navy">{product.totalSold}</span>
                <span>Terjual</span>
              </div>
            </>
          )}
      ...
      <div className="space-y-6 mb-10">
        ...
        <div className="flex gap-4 items-center">
           <div className="w-24 flex-shrink-0 text-slate-500 text-sm font-sans font-medium">SKU</div>
        ...
        <div className="flex gap-4">
          <div className="w-24 flex-shrink-0 text-slate-500 text-sm font-sans font-medium mt-1">Pengiriman</div>
          <div className="flex-1">
             <div className="flex items-center gap-2 mb-1.5">
                <TruckIcon className="w-5 h-5 text-emerald-600" />
                <span className="text-emerald-700 font-bold font-sans text-sm">Pengiriman Gratis</span>
             </div>
             <p className="text-sm font-sans text-slate-500">Ongkos kirim Rp0 untuk pesanan di atas Rp500rb</p>
          </div>
        </div>

        {/* Quantity */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 pt-2">
          <div className="w-24 flex-shrink-0 text-slate-900 sm:text-slate-500 text-sm font-sans font-bold sm:font-medium mb-1 sm:mb-0">Kuantitas</div>
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="flex items-center border border-slate-200 rounded-xl overflow-hidden shadow-sm bg-white h-10">
              ...
              <button
                ...
                disabled={isPending || (matchedVariant?.stock !== undefined && quantity >= matchedVariant.stock)}
              >
       ...
       <div className="flex flex-row items-center gap-2 sm:gap-3 mb-10 w-full">
         <div className="flex flex-row gap-2 sm:gap-3 flex-1 w-full">
           <Button
             ...
             disabled={!(matchedVariant || product.variants[0])?.availableForSale || isPending}
             size="lg"
             variant="secondary"
      ...
      <div className="grid grid-cols-3 gap-2 py-5 border-t border-slate-200 text-xs font-sans font-bold text-mitologi-navy">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-2 text-center group">
          ...
          <span>Garansi<br className="sm:hidden"/> Mitologi</span>
        </div>
