...
export function StaticPageShell({
  ...
  return (
    <div className="min-h-screen bg-app-background pt-10">
      <div className={`mx-auto ${containerClass} px-4 sm:px-6 lg:px-12 xl:px-20 pb-16 md:pb-24`}>
            ...
            <nav className="mb-4 flex items-center gap-2 text-[11px] font-sans font-semibold uppercase tracking-[0.18em] text-slate-500">
              ...
              {breadcrumbs.map((crumb, i) => (
                <span key={i} className="flex items-center gap-2">
                  ...
                  {crumb.href ? (
                    <Link href={crumb.href} className="hover:text-mitologi-navy transition-colors">
                      {crumb.label}
                    </Link>
                  ) : (
                    <span className="text-mitologi-navy font-bold">{crumb.label}</span>
                  )}
                </span>
              ))}
            </nav>
        ...
        {/* Content Card */}
        <MotionDiv delay={0.1} className="bg-white rounded-[28px] border border-app p-6 sm:p-8 md:p-12 shadow-soft">
          {children}
        </MotionDiv>
      </div>
    </div>
