...
export async function RecommendationsSection() {
  ...
  if (user) {
    try {
        products = await getRecommendations(Number(user.id));
    } catch (e) {
        console.error("Failed to fetch recommendations:", e);
    }
  }

  // Fallback if no user or no personalized data (optional: fetch best sellers or trending)
  // For now return null as per original logic if empty
      ...
      <div className="relative">
        ...
        <div className="flex overflow-x-auto pb-6 -mx-4 px-4 sm:hidden snap-x snap-mandatory scrollbar-hide">
            {products.slice(0, 4).map((product, index) => (
                <div key={product.id} className="min-w-[260px] max-w-[260px] flex-shrink-0 mr-4 snap-center">
                    <ProductCard product={product} index={index} />
                </div>
            ))}
             <div className="min-w-[140px] flex items-center justify-center snap-center">
                <Link href="/shop" className="flex flex-col items-center gap-3 text-slate-500 hover:text-mitologi-navy transition-colors group">
