...
export function ProductFilters({ categories, activeCategory: propActiveCategory }: ProductFiltersProps) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  let activeCategory = propActiveCategory !== undefined ? propActiveCategory : (searchParams.get("category") || null);
    ...
    <div className="space-y-8">
        ...
        <div className="space-y-3">
          ...
          >
            <span className={cx(
                ...
                !activeCategory ? "bg-mitologi-gold" : "bg-transparent group-hover:bg-slate-300"
            )} />
            Semua Produk
          </button>

          {categories
            .filter((category) => !["", "hidden-homepage-featured-items", "hidden-homepage-carousel"].includes(category.handle))
            .map((category) => {
             ...
             return (
                ...
                >
                    <span className={cx(
                        ...
                        isActive ? "bg-mitologi-gold" : "bg-transparent group-hover:bg-slate-300"
                    )} />
                    {category.title}
                </button>
             );
      ...
      <div className="mt-8">
        ...
        <div className="space-y-4">
             <div className="space-y-3">
                 <div>
                     <label htmlFor="minPrice" className="text-xs font-sans font-bold text-slate-500 mb-1.5 block">Minimal</label>
                     <div className="relative">
                         <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-sans font-medium">Rp</span>
                         <Input
                            type="number"
                            id="minPrice"
                            placeholder="0"
                 ...
                 <div>
                     <label htmlFor="maxPrice" className="text-xs font-sans font-bold text-slate-500 mb-1.5 block">Maksimal</label>
                     <div className="relative">
                         <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-sans font-medium">Rp</span>
                         <Input
                            type="number"
