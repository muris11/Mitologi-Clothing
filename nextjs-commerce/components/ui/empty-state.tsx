...
function EmptyState({ icon, title, description, action, className, ...props }: EmptyStateProps) {
  return (
    <div 
      className={cn("flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50/50 p-12 text-center", className)}
      {...props}
    >
      ...
      <h3 className="font-sans font-bold text-2xl tracking-tight text-mitologi-navy">{title}</h3>
      {description && (
