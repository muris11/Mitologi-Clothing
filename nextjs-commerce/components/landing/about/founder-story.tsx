...
export function FounderStory({ settings }: FounderStoryProps) {
  const name = settings?.about?.founder_name;
  const role = settings?.about?.founder_role || "Founder & CEO";
  const story = settings?.about?.founder_story;
  const photo = settings?.about?.founder_photo;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            ...
            <div className="absolute bottom-12 -left-8 bg-white p-6 rounded-2xl shadow-xl border border-slate-100 z-20 transform -rotate-3 hover:rotate-0 transition-transform duration-300">
              <span className="block text-3xl font-bold text-mitologi-navy mb-1 leading-none">10+</span>
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Tahun<br/>Pengalaman</span>
            </div>
          </motion.div>
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold font-sans tracking-tight text-mitologi-navy mb-8 leading-[1.1]">
              Dari Sebuah Ide Kecil Menjadi <span className="text-mitologi-gold">Standar Kualitas</span>.
            </h2>
