...
function MemberCard({ member, isFounder = false }: { member: TeamNode; isFounder?: boolean }) {
  return (
    <div className="flex flex-col items-center">
      {/* Photo — natural size, no effects */}
      <div className={`overflow-hidden bg-white rounded-2xl shrink-0
        ${isFounder ? 'border border-mitologi-navy/20 shadow-md' : 'border border-slate-100 shadow-sm'}`}
      >
        {member.photo_url ? (
          <img
            src={storageUrl(member.photo_url)}
            alt={member.name}
             className={`block object-contain ${isFounder ? 'w-32 md:w-64' : 'w-24 md:w-48'}`}
          />
        ) : (
          <div className={`flex items-center justify-center bg-slate-50 text-mitologi-navy
            ${isFounder ? 'w-32 h-32 md:w-64 md:h-64' : 'w-24 h-24 md:w-48 md:h-48'}`}
          >
            <span className={`font-sans font-bold ${isFounder ? 'text-4xl md:text-7xl' : 'text-2xl md:text-5xl'}`}>
      ...
      {/* Name & Position */}
      <h4 className={`font-sans font-bold text-mitologi-navy text-center mt-4 md:mt-5 leading-tight tracking-tight
        ${isFounder ? 'text-sm md:text-xl' : 'text-[13px] md:text-base'}`}
      >
        {member.name}
      </h4>
      <p className={`text-slate-500 font-sans font-medium text-center mt-1 md:mt-1.5 uppercase tracking-widest
...
function TreeLevel({ nodes, isRoot = false }: { nodes: TeamNode[]; isRoot?: boolean }) {
  if (nodes.length === 0) return null;

  return (
    <div className="flex flex-col items-center">
      {/* Members at this level */}
      <div className={`flex flex-nowrap justify-center ${isRoot ? 'gap-6 md:gap-12' : 'gap-4 md:gap-8'}`}>
        {nodes.map((node) => (
      ...
      <div className="mx-auto max-w-[1440px] px-6 lg:px-8 relative z-10">
        ...
        <div className="mx-auto max-w-3xl text-center mb-20">
          ...
          <p className="text-slate-600 font-sans font-medium text-base md:text-lg">
            Kenali tim profesional di balik setiap produk berkualitas dari perusahaan kami.
          </p>
        </div>
