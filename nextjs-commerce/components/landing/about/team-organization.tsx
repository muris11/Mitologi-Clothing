...
const TeamCard = ({ name, role, image, isLeader = false }: { name: string, role: string, image: string, isLeader?: boolean }) => (
    <div className={`flex flex-col items-center text-center group ${isLeader ? 'z-10' : ''}`}>
        <div className={`relative overflow-hidden rounded-2xl bg-white border border-slate-100 shadow-soft transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-hover mb-6 ${isLeader ? 'w-48 h-64 md:w-56 md:h-72 ring-4 ring-mitologi-navy/5 ring-offset-4' : 'w-40 h-52 md:w-48 md:h-64'}`}>
            <Image 
                src={image} 
                alt={name}
                fill
        ...
        <h3 className={`font-sans font-bold text-mitologi-navy tracking-tight ${isLeader ? 'text-lg md:text-xl' : 'text-base'}`}>
            {name}
        </h3>
        <p className={`text-xs tracking-widest uppercase mt-2 font-sans ${isLeader ? 'text-mitologi-gold font-bold' : 'text-slate-500 font-bold'}`}>
            {role}
        </p>
    </div>
);
