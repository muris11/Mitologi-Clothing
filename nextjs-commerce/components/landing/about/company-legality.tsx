...
export function CompanyLegality({ settings }: { settings?: SiteSettings }) {
  ...
  const items = [
    { label: 'Nama Badan Usaha', value: legality?.legal_company_name || fallback?.legal_company_name },
    { label: 'Alamat Resmi', value: legality?.legal_address || fallback?.legal_address },
    { label: 'Bidang Usaha', value: legality?.legal_business_field || fallback?.legal_business_field },
    { label: 'NPWP', value: legality?.legal_npwp || fallback?.legal_npwp },
    { label: 'NIB', value: legality?.legal_nib || fallback?.legal_nib },
    { label: 'NMID', value: legality?.legal_nmid || fallback?.legal_nmid },
    ...
    <MotionSection className="py-20 lg:py-24 bg-white border-t border-slate-200">
      <div className="mx-auto max-w-[1440px] px-6 lg:px-8">
        ...
        <StaggerGrid className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {items.map((item, idx) => (
            <StaggerGridItem key={idx} className="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100 hover:border-slate-200 hover:shadow-soft transition-all group">
