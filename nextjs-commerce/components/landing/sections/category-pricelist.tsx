...
export function CategoryPricelist({ pricings }: { pricings?: ProductPricing[] }) {
                  ...
                  <div className="mb-8">
                    {pricing.min_order && (
                      <span className="inline-block px-4 py-1.5 rounded-full bg-mitologi-navy/5 text-mitologi-navy font-bold text-xs uppercase tracking-widest font-sans border border-mitologi-navy/10 mb-6">
                        Minimal Order: {pricing.min_order}
                      </span>
                    )}
                    
                    <div className="space-y-4">
                      {pricing.items && Array.isArray(pricing.items) && pricing.items.map((item, itemIdx) => (
                        <div key={itemIdx} className="flex justify-between items-center py-4 border-b border-dashed border-slate-200 group hover:border-mitologi-gold/50 transition-colors">
                  ...
                  {pricing.notes && (
                    <div className="bg-mitologi-cream p-5 rounded-2xl flex items-start gap-4">
                      <div className="w-8 h-8 rounded-full bg-mitologi-gold/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                        <span className="text-mitologi-navy font-bold text-lg leading-none">!</span>
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-mitologi-gold uppercase tracking-widest mb-1 font-sans">Catatan</span>
