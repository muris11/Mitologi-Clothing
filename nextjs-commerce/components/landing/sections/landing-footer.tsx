...
export default function LandingFooter({ settings }: { settings?: SiteSettings }) {
  const socialLinks = [
      ...
      <MotionDiv className="container mx-auto px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-20">
          ...
          <div className="lg:col-span-4">
            <Link href="/" className="flex flex-col mb-8 group">
              <span className="text-3xl font-sans font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-200 group-hover:from-mitologi-gold group-hover:to-mitologi-gold/80 transition-all duration-300">
                {settings?.general?.site_name || "Mitologi"}
              </span>
            ...
            <p className="text-slate-400 mb-8 leading-relaxed max-w-sm font-medium">
              {settings?.general?.site_description || "Mitologi Clothing Premium."}
            </p>
          ...
          {footerLinks.map((column) => (
            <div key={column.title} className="lg:col-span-2">
              <h3 className="text-sm font-bold mb-6 text-white uppercase tracking-widest">{column.title}</h3>
          ...
          <div className="lg:col-span-3">
            <h3 className="text-sm font-bold mb-6 text-white uppercase tracking-widest">Hubungi Kami</h3>
            ...
            <div className="mt-8">
              {(settings?.contact?.contact_whatsapp || settings?.contact?.whatsapp_number) && (
                <a 
                  href={`https://wa.me/${settings?.contact?.contact_whatsapp || settings?.contact?.whatsapp_number}`}
                  target="_blank"
                  rel="noopener noreferrer"
        ...
        <div className="border-t border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-sm">
          <p className="text-slate-400 text-center md:text-left">
            &copy; {new Date().getFullYear()} {settings?.general?.site_name || "Mitologi Clothing"}. All rights reserved.
          </p>
          <div className="flex gap-6 text-slate-400">
             <Link href="/privacy-policy" className="hover:text-mitologi-gold hover:underline underline-offset-4 transition-colors duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mitologi-gold rounded-sm">Privacy Policy</Link>
             <Link href="/terms-of-service" className="hover:text-mitologi-gold hover:underline underline-offset-4 transition-colors duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mitologi-gold rounded-sm">Terms of Service</Link>
          </div>
        </div>
      </MotionDiv>
    </footer>
