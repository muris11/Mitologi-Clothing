...
export function OrderFlow({ orderSteps = [] }: { orderSteps?: OrderStep[] }) {
    const [activeTab, setActiveTab] = useState<'langsung' | 'ecommerce'>('langsung');

    if (orderSteps.length === 0) return null;

    const filteredSteps = orderSteps
        .filter(step => step.type === activeTab || (!step.type && activeTab === 'langsung')) // Fallback for legacy data
            ...
            <div className="max-w-3xl mx-auto">
                <div className="p-8 md:p-12 min-h-[400px] border border-slate-100 rounded-3xl shadow-soft bg-white relative overflow-hidden">
                    ...
                    <div className="space-y-12 relative z-10">
                        ...
                        {filteredSteps.length > 0 ? filteredSteps.map((step, idx) => (
                             <div key={step.id} className="flex gap-6 relative group">
                                 ...
                                 <div className="flex-auto py-2">
                                     <h4 className="text-xl font-sans font-bold text-mitologi-navy mb-2 tracking-tight group-hover:text-mitologi-gold transition-colors duration-300">{step.title}</h4>
                                     <p className="text-sm text-slate-600 leading-relaxed font-sans font-medium">{step.description}</p>
                                 </div>
                             </div>
                        )) : (
