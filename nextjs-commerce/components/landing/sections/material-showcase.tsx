...
export function MaterialShowcase({ materials = [] }: { materials?: Material[] }) {
      ...
      <div className="mx-auto max-w-[1440px] px-6 lg:px-8">
        ...
        <StaggerGrid className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {materials.map((material) => {
              const themeClass = material.color_theme && COLOR_THEMES[material.color_theme] 
                ? COLOR_THEMES[material.color_theme] 
                : 'bg-mitologi-navy/5 text-mitologi-navy';
                
              return (
              <StaggerGridItem key={material.id}>
                <Card 
                  className="group relative p-5 sm:p-8 flex flex-col justify-between rounded-2xl border border-slate-100 bg-white shadow-soft hover:shadow-hover transition-all duration-300 hover:-translate-y-1 overflow-hidden h-full"
                >
                 {/* Top border effect */}
                ...
                <div className="relative z-20">
                  {material.color_theme && (
                    <div className="mb-5">
                      <span className={`inline-flex items-center rounded-full px-3 py-1 text-[10px] sm:text-xs font-bold font-sans uppercase tracking-widest ${themeClass}`}>
                          {material.name}
                      </span>
                    </div>
                  )}
