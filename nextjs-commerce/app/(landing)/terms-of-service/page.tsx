...
export default async function TermsOfServicePage() {
  const data = await getLandingPageData();
  const siteName = data.site_settings?.general?.site_name || 'Mitologi Clothing';
  const email = data.site_settings?.contact?.contact_email || 'mitologiclothing@gmail.com';
  const phone = data.site_settings?.contact?.contact_phone || '+62 813-2217-0902';
  const address = data.site_settings?.contact?.contact_address || 'Desa Leuwigede Kec. Widasari Kab. Indramayu 45271';
  const whatsapp = data.site_settings?.contact?.whatsapp_number || '6281322170902';
  ...
  const sections = [
    ...
    },
    {
      ...
      extra: 'Klaim garansi harus diajukan maksimal 3 hari setelah barang diterima, disertai foto dan video bukti kerusakan/ketidaksesuaian.',
    },
    {
    ...
    },
    {
      ...
      content: [
        `${siteName} tidak bertanggung jawab atas:`,
      ],
      list: [
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center gap-3 mb-12 pb-8 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-500">
                Terakhir diperbarui: <span className="font-semibold text-slate-800">18 Februari 2026</span>
              </p>
            </div>
            ...
            <div className="mb-12">
              <p className="text-slate-600 leading-relaxed text-lg">
                Selamat datang di <span className="font-semibold text-mitologi-navy">{siteName}</span>.
                Syarat & Ketentuan berikut mengatur penggunaan website, pemesanan produk, dan semua layanan 
                yang kami sediakan. Mohon baca dengan seksama sebelum melakukan transaksi.
              </p>
            </div>
              ...
              {sections.map((section, idx) => (
                <article key={idx} className="group">
                  ...
                  {section.content.map((paragraph, pIdx) => (
                    <p key={pIdx} className="text-slate-600 leading-relaxed mb-4 pl-4">
                      {paragraph}
                    </p>
                  ))}
                  ...
                  {section.subsections && section.subsections.map((sub, sIdx) => (
                    <div key={sIdx} className="mt-6 ml-4">
                      <h3 className="text-lg font-semibold text-mitologi-navy/80 mb-3">{sub.subtitle}</h3>
            ...
            <div className="mt-16 p-8 rounded-3xl bg-gradient-to-br from-mitologi-navy to-[#1a1a3e] text-white shadow-2xl shadow-mitologi-navy/20">
              ...
              <p className="text-slate-300 mb-8 leading-relaxed">
                Jika Anda memiliki pertanyaan mengenai Syarat & Ketentuan ini, jangan ragu untuk menghubungi kami:
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center flex-shrink-0">
                    <svg className="w-6 h-6 text-mitologi-gold" fill="none" viewBox="0 0 24 24" stroke="currentColor">
