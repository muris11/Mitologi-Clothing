...
const kaosJerseyData = {
  title: "Kaos & Jersey",
  description: "Ukuran standar untuk kaos polos, kaos sablon, dan jersey olahraga.",
    ...
    >
            ...
            <div className="w-full lg:w-80 flex-shrink-0">
              <div className="relative bg-gradient-to-br from-slate-50 to-slate-100 rounded-3xl p-8 flex items-center justify-center aspect-square border border-slate-200/60">
                <div className="relative">
                  ...
                  <svg viewBox="0 0 200 220" className="w-48 h-48 text-slate-300" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M60 30 L30 60 L50 70 L50 200 L150 200 L150 70 L170 60 L140 30 L120 50 C110 58 90 58 80 50 L60 30Z" stroke="currentColor" strokeWidth="2" fill="white"/>
                    {/* Lebar Dada arrow */}
                    <line x1="55" y1="95" x2="145" y2="95" stroke="#1e3a5f" strokeWidth="1.5" strokeDasharray="4 3"/>
                    <polygon points="55,95 60,92 60,98" fill="#1e3a5f"/>
                    <polygon points="145,95 140,92 140,98" fill="#1e3a5f"/>
                    {/* Panjang arrow */}
                    <line x1="100" y1="40" x2="100" y2="195" stroke="#1e3a5f" strokeWidth="1.5" strokeDasharray="4 3"/>
                    <polygon points="100,40 97,45 103,45" fill="#1e3a5f"/>
                    <polygon points="100,195 97,190 103,190" fill="#1e3a5f"/>
                  </svg>
                  {/* Labels */}
                  <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[60%] bg-white/90 px-2 py-0.5 rounded text-[10px] font-bold text-mitologi-navy whitespace-nowrap shadow-sm border border-slate-100">Lebar Dada</span>
                  <span className="absolute top-1/2 right-[-38px] -translate-y-1/2 bg-white/90 px-2 py-0.5 rounded text-[10px] font-bold text-mitologi-navy writing-mode-vertical shadow-sm border border-slate-100" style={{ writingMode: "vertical-rl" }}>Panjang</span>
                </div>
              </div>
            </div>
                ...
                <table className="w-full min-w-[600px]">
                  <thead>
                    <tr className="bg-mitologi-navy text-white">
                      <th className="px-5 py-4 text-xs font-bold uppercase tracking-wider text-left border-r border-white/10">Size Chart</th>
                      {kaosJerseyData.sizes.map((s) => (
                        <th key={s} className="px-5 py-4 text-center text-sm font-bold border-r border-white/10 last:border-r-0">{s}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {kaosJerseyData.measurements.map((m, i) => (
                      <tr key={m} className={`border-b border-slate-100 last:border-0 ${i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                        ...
                        {kaosJerseyData.values[m as keyof typeof kaosJerseyData.values]!.map((v, j) => (
                          <td key={j} className="px-5 py-4 text-center text-sm font-semibold text-slate-600 border-r border-slate-100 last:border-r-0">
                            {v} <span className="text-slate-400 text-xs">cm</span>
                          </td>
                        ))}
                      </tr>
                    ))}
        ...
        <section className="mb-16 animate-in fade-in duration-300">
          <div className="flex flex-col lg:flex-row gap-10 items-start">
            ...
            <div className="w-full lg:w-80 flex-shrink-0">
              <div className="relative bg-gradient-to-br from-slate-50 to-slate-100 rounded-3xl p-8 flex items-center justify-center aspect-square border border-slate-200/60">
                <div className="relative">
                  <svg viewBox="0 0 200 240" className="w-48 h-48 text-slate-300" fill="none" xmlns="http://www.w3.org/2000/svg">
                    {/* Kemeja outline */}
                    <path d="M70 25 L40 45 L25 110 L55 100 L55 220 L145 220 L145 100 L175 110 L160 45 L130 25 L115 45 C108 55 92 55 85 45 L70 25Z" stroke="currentColor" strokeWidth="2" fill="white"/>
                    {/* Placket line */}
                    <line x1="100" y1="45" x2="100" y2="220" stroke="currentColor" strokeWidth="1"/>
                    {/* Buttons */}
                    ...
                    {/* Lebar arrow */}
                    <line x1="58" y1="110" x2="142" y2="110" stroke="var(--color-mitologi-gold)" strokeWidth="1.5" strokeDasharray="4 3"/>
                    <polygon points="58,110 63,107 63,113" fill="var(--color-mitologi-gold)"/>
                    <polygon points="142,110 137,107 137,113" fill="var(--color-mitologi-gold)"/>
                    {/* Tinggi arrow */}
                    <line x1="155" y1="30" x2="155" y2="216" stroke="var(--color-mitologi-gold)" strokeWidth="1.5" strokeDasharray="4 3"/>
                    <polygon points="155,30 152,35 158,35" fill="var(--color-mitologi-gold)"/>
                    <polygon points="155,216 152,211 158,211" fill="var(--color-mitologi-gold)"/>
                  </svg>
                  <span className="absolute top-[42%] left-1/2 -translate-x-1/2 bg-white/90 px-2 py-0.5 rounded text-[10px] font-bold text-mitologi-navy shadow-sm border border-slate-100">Lebar</span>
                  <span className="absolute top-1/2 right-[-24px] -translate-y-1/2 bg-white/90 px-2 py-0.5 rounded text-[10px] font-bold text-mitologi-navy shadow-sm border border-slate-100" style={{ writingMode: "vertical-rl" }}>Tinggi</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                {kemejaCategories.map((cat) => (
                  <div key={cat.title} className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-slate-50 text-slate-600">
                          <th className="px-4 py-2.5 text-xs font-bold uppercase text-left border-b border-r border-slate-200">Size</th>
                          <th className="px-4 py-2.5 text-xs font-bold uppercase text-center border-b border-r border-slate-200">Lebar</th>
                          <th className="px-4 py-2.5 text-xs font-bold uppercase text-center border-b border-slate-200">Tinggi</th>
                        </tr>
                      </thead>
                      <tbody>
                        {cat.sizes.map((size, i) => (
                          <tr key={size} className={`border-b border-slate-100 last:border-0 ${i % 2 === 1 ? "bg-slate-50/40" : ""}`}>
                            <td className="px-4 py-2.5 text-sm font-bold text-slate-800 border-r border-slate-100">{size}</td>
                            <td className="px-4 py-2.5 text-sm font-medium text-slate-600 text-center border-r border-slate-100">{cat.lebar[i]} cm</td>
                            <td className="px-4 py-2.5 text-sm font-medium text-slate-600 text-center">{cat.tinggi[i]} cm</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <section className="mb-8">
        ...
        <div className="space-y-4">
          {tips.map((tip, i) => (
            <div key={i} className="flex items-start gap-5 p-5 rounded-2xl bg-white border border-slate-100 shadow-sm hover:border-mitologi-navy/20 transition-colors">
              ...
              <p className="text-sm font-medium text-slate-700 leading-relaxed pt-2.5">{tip}</p>
            </div>
          ))}
        </div>
      </section>
