...
export async function generateMetadata({
  ...
  if (searchValue) {
    title = `Pencarian: ${searchValue}`;
     ...
     const catName = typeof categoryHandle === 'string' 
  ...
  return {
    ...
    description: "Temukan koleksi kain dan pakaian premium dari Mitologi Clothing.",
    openGraph: {
      title: `${title} - Mitologi Clothing`,
      images: [
        {
...
async function ProductList({ 
  sort, searchValue, categoryHandle, minPrice, maxPrice, page, limit 
}: { 
  sort: string, searchValue?: string, categoryHandle?: string, minPrice?: number, maxPrice?: number, page: number, limit: number 
}) {
    ...
    <>
      ...
      {products.length === 0 ? (
         <div className="text-center py-24 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center justify-center">
            ...
            <p className="text-slate-500 font-sans text-sm max-w-md mx-auto">Maaf, kami tidak dapat menemukan produk yang sesuai dengan pencarian atau filter Anda. Coba kata kunci lain.</p>
         </div>
      ) : (
         <StaggerGrid className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
...
export default async function ShopPage(props: ShopPageProps) {
  ...
  const categoryHandle = typeof searchParams.category === "string" ? searchParams.category : undefined;
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-app pb-6">
              <div>
                  ...
                  <h1 className="font-display text-4xl md:text-5xl font-semibold tracking-tight text-mitologi-navy leading-none">
                   {searchValue ? `Pencarian: ${searchValue}` : categoryHandle ? `Kategori: ${categoryHandle}` : 'Katalog Koleksi'}
                  </h1>
                  <p className="text-slate-500 text-sm md:text-base mt-3 font-sans max-w-2xl leading-relaxed">
                  Pilihan produk Mitologi yang ditata lebih tenang, lebih fokus, dan nyaman dijelajahi di layar besar maupun kecil.
                  </p>
              </div>
          ...
          <div className="flex flex-col lg:flex-row gap-10">
             ...
             {/* Desktop Sidebar */}
              <div className="w-full lg:w-72 flex-shrink-0 hidden lg:block">
                 <div className="sticky top-28 rounded-[24px] border border-app bg-white p-5 shadow-soft">
                   <ProductFilters categories={collections} activeCategory={categoryHandle || null} />
                 </div>
              </div>
             ...
             <div className="flex-1">
                 <div className="flex items-center justify-between mb-5 lg:hidden">
                     <MobileFilters categories={collections} activeCategory={categoryHandle || null} />
                 </div>
