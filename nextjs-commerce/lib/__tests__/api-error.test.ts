...
describe('ApiError', () => {
  ...
  it('stores validation errors', () => {
    const errors = { email: ['Email sudah terdaftar'], name: ['Nama wajib diisi'] }
  ...
  it('isEmailTaken detects taken email (English)', () => {
    const error = new ApiError('', 422, { email: ['The email has already been taken.'] })
    expect(error.isEmailTaken()).toBe(true)
  })

  it('isEmailTaken detects taken email (Indonesian)', () => {
    const error = new ApiError('', 422, { email: ['Email sudah terdaftar'] })
    expect(error.isEmailTaken()).toBe(true)
  })

  it('isEmailTaken returns false for unrelated errors', () => {
    const error = new ApiError('', 422, { email: ['Format email salah'] })
