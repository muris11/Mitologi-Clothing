...
const internalApiUrl = process.env.INTERNAL_API_URL || publicApiUrl || defaultApiUrl;
const normalizedPublicApiUrl = publicApiUrl.includes("__HOST__") ? defaultApiUrl : publicApiUrl;
const projectRoot = path.dirname(fileURLToPath(import.meta.url));

const toBackendOrigin = (apiUrl: string) => apiUrl.replace(/\/api\/?$/, "").replace(/\/+$/, "");
...
const backendOrigins = Array.from(new Set([backendPublicOrigin, backendInternalOrigin]));
...
const backendRemotePatterns = backendOrigins
    .map(toRemotePattern)
    .filter((pattern): pattern is { protocol: string; hostname: string; port: string | undefined } => Boolean(pattern));
...
export default {
    ...
    images: {
        ...
        contentSecurityPolicy: "default-src 'self'; script-src 'none'; style-src 'none'; sandbox;",
        remotePatterns: [
